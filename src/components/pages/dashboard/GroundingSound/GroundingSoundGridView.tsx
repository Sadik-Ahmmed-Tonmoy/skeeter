...
const GroundingSoundGridView = () => {
  ...
  const { data, isLoading, isFetching } = useGetAllGroundQuery(objectQuery, {
    skip: !objectQuery.length,
  });
  const [deleteGroundMutation] = useDeleteGroundMutation();
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      const response = await handleAsyncWithToast(async () => {
        return deleteGroundMutation(id);
      }, "Deleting ground...");

      if (response?.data?.success) {
        setPage(1);
        Swal.fire("Deleted!", "Your ground has been deleted.", "success");
      }
    }
  };
    ...
    <div>
      {data?.result?.data?.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 mb-8">
          {data?.result?.data?.map((ground: any) => (
            <div
              key={ground.id}
            ...
            >
              <div className="relative">
                <Image
                  src={ground?.soundImage}
                  alt={ground.soundName}
                  width={200}
                  height={200}
                  className="w-full h-44 object-fill"
                  // unoptimized
                  blurDataURL={ground?.soundImage}
                />
                <div className="absolute top-2 right-2 flex space-x-1">
                  <Link href={`/dashboard/grounding-sound/${ground.id}`}>
              ...
              <div className="p-3">
                ...
                <p className="text-xs text-gray-500">{ground.time}</p>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <div className="text-center py-6">No grounding sounds found</div>
      )}
...
export default GroundingSoundGridView;
