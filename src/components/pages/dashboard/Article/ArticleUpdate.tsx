...
export default function ArticleUpdate() {
  ...
  // State to track if user wants to replace existing image
  const [showImageUpload, setShowImageUpload] = useState(false);
  const [hasExistingImage, setHasExistingImage] = useState(false);

  const { data: articleData, isLoading } = useGetSingleArticleQuery(articleId, {
    skip: !articleId,
  });
  ...
  useEffect(() => {
    if (articleData?.result) {
      ...
      // Set form values
      setValue("mood", article.mood[0] || "");
      setValue("bringsYouHere", article.goal[0] || "");
      setValue("mentalHealthIssue", article.cause[0] || "");
      setValue("emotionallyOverwhelmed", article.emotionalReason[0] || "");
      setValue("title", article.title || "");
      setValue("time", article.time || "");

      // Set sections
      if (article.content?.sections && article.content.sections.length > 0) {
        const sectionsData = article.content.sections.map((section: any) => ({
          sectionTitle: section.sectionTitle || "",
          description: section.description || "",
        }));
        replace(sectionsData);
      }

      // Check if article has existing image
  ...
  const handleRemoveImage = () => {
    setValue("image", null);
    // If there's an existing image, go back to showing it
    if (hasExistingImage) {
      setShowImageUpload(false);
    }
  };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    formDataToSend.append("bodyData", JSON.stringify(body));

    // Only append image if user has uploaded a new one
    if (data.image && data.image instanceof File) {
      formDataToSend.append("articleImage", data.image);
    }

    const response = await handleAsyncWithToast(async () => {
      return updateArticleMutation({ id: articleId, formData: formDataToSend });
    }, "Updating article...");
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        {...field}
                        value={field.value || undefined}
                        placeholder="What makes you feel emotionally overwhelmed?"
                        status={
              ...
              <div className="space-y-4">
                ...
                <div>
                  {fields.map((field, index) => (
                    <div
                      key={field.id}
                      className="space-y-4 border border-gray-200 p-4 rounded-lg mb-4"
                    >
                  ...
                  <div className="flex justify-end">
                    <Button
                      type="primary"
                      onClick={() =>
                        append({ sectionTitle: "", description: "" })
                      }
                      className="w- bg-teal-600 hover:bg-teal-700 text-white py-3 px-4 rounded-md text-base font-medium h-auto"
                ...
                <div>
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-gray-400 transition-colors">
                    ...
                    {formData.image && showImageUpload ? (
                    ...
                    hasExistingImage &&
                      !showImageUpload &&
                      articleData?.result?.articleImage ? (
                      <div className="relative">
                        <Image
                          src={articleData.result.articleImage}
